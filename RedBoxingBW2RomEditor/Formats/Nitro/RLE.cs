    ...
    public class RLE : NitroCFormat
    {
        ...
        public override long Decompress(Stream instream, long inLength, Stream outstream)
        {
            ...
            if (type != base.magicByte)
                throw new InvalidDataException(String.Format("The provided stream is not a valid RLE compressed stream. Invalid type 0x{0}", type.ToString("X")));
            ...
            while (currentOutSize < decompressedSize)
            {
                ...
                if (compressed)
                {
                    ...
                    if (currentOutSize + length > decompressedSize)
                        throw new InvalidDataException("The given stream is not a valid RLE stream; the output length does not match the provided plaintext length.");
