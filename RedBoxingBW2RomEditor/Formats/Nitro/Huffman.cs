    ...
    public class Huffman : NitroCFormat
    {
        ...
        public override long Decompress(Stream instream, long inLength, Stream outstream)
        {
            ...
            */
            ...
            if (type != (byte)blockSize)
                throw new InvalidDataException(String.Format("The provided stream is not a valid Huffman compressed stream. Invalid type 0x{ 0 }; unknown block size.", type.ToString("X")));
            ...
            while (currentSize < decompressedSize)
            {
                ...
                switch (blockSize)
                {
                    ...
                    default:
                        throw new Exception(String.Format("Unknown block size {0}", blockSize.ToString()));
                }
                #endregion
        ...
        public override int Compress(Stream instream, long inLength, Stream outstream)
        {
            switch (CompressBlockSize)
            {
                ...
                default:
                    throw new Exception(String.Format("Unhandled blocksize {0}", CompressBlockSize));
            }
        }
