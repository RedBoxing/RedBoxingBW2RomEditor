...
using RedBoxingBW2RomEditor.Exceptions;

namespace RedBoxingBW2RomEditor.Formats.Nitro
{
    /// <summary>
    ...
    public class LZ11 : NitroCFormat
    {
        ...
        public override long Decompress(Stream instream, long inLength, Stream outstream)
        {
            ...
            if (type != base.magicByte)
                throw new InvalidDataException(String.Format("The provided stream is not a valid LZ-0x11 compressed stream. Invalid type 0x{1}", type.ToString("X")));
            ...
            while (currentOutSize < decompressedSize)
            {
                ...
                if ((flags & mask) > 0)
                {
                    ...
                    if (disp > currentOutSize)
                        throw new InvalidDataException(String.Format("Cannot go back more than already written. DISP = {0}, #written bytes = 0x{1} before 0x{2} with indicator 0x{3}", disp, currentOutSize.ToString("X"),
